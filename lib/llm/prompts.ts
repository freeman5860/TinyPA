...
输出规则（严格）：
- 一行输出一个 JSON 对象，一行一条，不要数组包裹，不要 markdown 代码块，不要额外解释文字。
- 每行形如：{"type":"todo","content":"...","due_at":"...","priority":2,"tags":[...]}
  其中 due_at / priority / tags 是可选字段。
- 如果没什么可抽的，整个输出留空。
- 每条消息通常抽出 1-5 个 items，最多不超过 10 个。
