...
Cron 要等到整点，想立刻验证就手工触发：

```bash
# 用你的生产域名 + CRON_SECRET
curl -H "Authorization: Bearer $CRON_SECRET" \
  "https://tinypa-xxx.vercel.app/api/cron/digest?force=1"

curl -H "Authorization: Bearer $CRON_SECRET" \
  "https://tinypa-xxx.vercel.app/api/cron/morning?force=1"
```

- 第一条返回 `{"count":1,"results":[{"userId":"...","ran":true}]}` → 到 "复盘" 页能看到今天那张卡。
- 第二条返回 `sent:true` → 邮箱查收早报。

`force=1` 会跳过"是否到小时"的判断，仅限调试用。

### 5.4 Cron 自动跑

`vercel.json` 里我配了两个 cron：

```json
{
  "crons": [
    { "path": "/api/cron/digest",  "schedule": "3 * * * *" },
    { "path": "/api/cron/morning", "schedule": "7 * * * *" }
  ]
}
```

**每小时**各跑一次（避开整点，省得撞 Vercel 调度高峰）。每个任务内部再按每个用户的 `digestHour` / `morningHour` + 时区判断要不要真的执行。

Vercel 会自动给 cron 请求加上 `Authorization: Bearer $CRON_SECRET` 头（前提是你设了同名环境变量），所以不用额外配置。

验证：Vercel 项目 → **Cron Jobs** 标签（或 **Logs** 里过滤 `/api/cron/`），能看到最近一次运行的 200 状态和返回体。

> **Vercel Hobby 限制**：免费档有 cron 条数和频次限制。如果你后面把 cron 细到每 10 分钟，或者要跑更多定时任务，换成 [Upstash QStash](https://upstash.com/docs/qstash/features/schedules) 免费 500 次/天。迁移时把 schedule 目标 URL 指向同样的 `/api/cron/*`，Authorization 头里也带 `Bearer $CRON_SECRET` 就行。
...
- 如果是刚部署，Cron 要等下一个整点才首次触发；想立刻试就用 5.3 的 `force=1` 手动触发
